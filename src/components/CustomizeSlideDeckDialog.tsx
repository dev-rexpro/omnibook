...
interface CustomizeSlideDeckDialogProps {
  open: boolean
...
export function CustomizeSlideDeckDialog({
  open,
  onOpenChange,
  onGenerate,
}: CustomizeSlideDeckDialogProps) {
  const [selectedFormat, setSelectedFormat] = useState("detailed-deck")
              ...
              {FORMAT_OPTIONS.map((option) => {
                ...
                return (
                  <div
                    ...
                    className={`border border-border rounded-2xl p-4 flex flex-col gap-2 cursor-pointer transition-all duration-200 relative group select-none min-h-[120px] ${
                      isActive
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex h-[44px] items-center border border-border rounded-xl p-1 bg-card w-full sm:w-auto self-start">
                {(["short", "default"] as const).map((len) => {
            ...
            <div className="border border-border rounded-2xl bg-card focus-within:ring-1 focus-within:ring-ring focus-within:border-ring transition flex flex-col overflow-hidden">
              ...
              <div className="text-[12px] leading-relaxed text-muted-foreground border-t border-border/50 p-4 pt-3 bg-muted/5">
                ...
                <ul className="list-disc pl-4 space-y-1.5 font-medium">
                  <li>Add a high-level outline, or guide the audience, style, and focus</li>
                  <li>"Create a deck for beginners using a bold and playful style with a focus on step-by-step instructions"</li>
                </ul>
              </div>
            </div>
          </div>
