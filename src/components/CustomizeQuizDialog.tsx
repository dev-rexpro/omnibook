...
export function CustomizeQuizDialog({
  ...
  const [selectedQuestionsCount, setSelectedQuestionsCount] = useState("Standard")
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex h-[44px] items-center border border-border rounded-xl p-1 bg-card w-full sm:w-auto self-start">
                {["Fewer", "Standard", "More"].map((option) => {
                  const isActive = selectedQuestionsCount === option
                  return (
                    <button
                      key={option}
                      onClick={() => setSelectedQuestionsCount(option)}
